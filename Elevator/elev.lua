...
local function printShadow(x, y, w)
    ...
    setCursorAll(x + w, y);
    writeAll(string.char(0x94))
    setCursorAll(x, y + 1);
    writeAll(string.char(0x82))
    for i = x, x + w - 1 do
        writeAll(string.char(0x83))
    end
    setCursorAll(x + w, y + 1);
    writeAll(string.char(0x81))
end
...
local function getFloor()
    if redstone.getInput(floorRed) then
        prvFloor = currentFloor
        currentFloor = 16 - redstone.getAnalogInput(floorRed)
    end
end
...
getFloor()
if not redstone.getInput(floorRed) then
    buttons[1].fn(1)
end
lightButton()
...
while true do
    ...
    if evType == "redstone" then
        if redstone.getInput(reqRed) then
            elevCall(16 - redstone.getAnalogInput(reqRed))
        end
        getFloor()
